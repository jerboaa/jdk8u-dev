...
public class TestSPISigned {
    ...
    public static void main(String[] args) throws Throwable {
        if (args.length == 1) {
            ...
            javaArgs.add("-Djava.ext.dirs=" + System.getProperty("java.home") + extDir + File.pathSeparator + MODS_DIR.toAbsolutePath().toString());
            javaArgs.add("-cp");
            String classPath = System.getProperty("java.class.path");
            javaArgs.add(classPath);
            javaArgs.add(TestSPISigned.class.getSimpleName());
            javaArgs.add("run-test");
            OutputAnalyzer out = ProcessTools.executeTestJvm(javaArgs.toArray(new String[]{}));
    ...
    private static <T> void check(T expected, T actual) {
        Asserts.assertEquals(expected, actual, "Expected calendar from SPI to be in effect. expected = " + expected + " actual = " + actual);
    }
